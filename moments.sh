...
if [ $# -eq 0 ]; then
    echo "Usage: $0 DATASET N_BOOTSTRAPS MAXL"
    exit 1
fi
...
echo "Calculating acceptance corrected moments for ${dataset} with ${n_bootstraps} bootstrapping instances. maxL=${maxL}" >&2
...
data_job_id=$(sbatch --parsable --kill-on-invalid-dep=yes --output="${output_directory}/${dataset}/${maxL}/full/logs/data.out" /N/u/rdube/Quartz/work/moments_workflow/calculate_measured_moments.slurm ${dataset} "data" ${maxL} "-1")
#submit unsampled accMC job
accMC_job_id=$(sbatch --parsable --kill-on-invalid-dep=yes --output="${output_directory}/${dataset}/${maxL}/full/logs/accMC.out" /N/u/rdube/Quartz/work/moments_workflow/calculate_measured_moments.slurm ${dataset} "accMC" ${maxL} "-1")
#submit unsampled genMC job
genMC_job_id=$(sbatch --parsable --kill-on-invalid-dep=yes --output="${output_directory}/${dataset}/${maxL}/full/logs/genMC.out" /N/u/rdube/Quartz/work/moments_workflow/calculate_measured_moments.slurm ${dataset} "genMC" ${maxL} "-1")
#submit unsampled acceptance matrix job
acceptance_matrix_job_id=$(sbatch --parsable --kill-on-invalid-dep=yes --output="${output_directory}/${dataset}/${maxL}/full/logs/acceptance_matrix.out" /N/u/rdube/Quartz/work/moments_workflow/acceptance_matrix.slurm ${dataset} ${maxL} "-1")

#submit unsampled acceptance correcting job
jobs_to_wait_for_acceptance_correcting="${data_job_id}:${accMC_job_id}:${genMC_job_id}:${acceptance_matrix_job_id}"
...
for i in $(seq 0 $((${n_bootstraps}-1))); do
    ...
    #submit unsampled data job
    data_job_id=$(sbatch --parsable --kill-on-invalid-dep=yes --output="${output_directory}/${dataset}/${maxL}/bootstrap_${i}/logs/data.out" /N/u/rdube/Quartz/work/moments_workflow/calculate_measured_moments.slurm ${dataset} "data" ${maxL} ${i})
    #submit unsampled accMC job
    accMC_job_id=$(sbatch --parsable --kill-on-invalid-dep=yes --output="${output_directory}/${dataset}/${maxL}/bootstrap_${i}/logs/accMC.out" /N/u/rdube/Quartz/work/moments_workflow/calculate_measured_moments.slurm ${dataset} "accMC" ${maxL} ${i})
    #submit unsampled genMC job
    genMC_job_id=$(sbatch --parsable --kill-on-invalid-dep=yes --output="${output_directory}/${dataset}/${maxL}/bootstrap_${i}/logs/genMC.out" /N/u/rdube/Quartz/work/moments_workflow/calculate_measured_moments.slurm ${dataset} "genMC" ${maxL} ${i})
    #submit unsampled acceptance matrix job
    acceptance_matrix_job_id=$(sbatch --parsable --kill-on-invalid-dep=yes --output="${output_directory}/${dataset}/${maxL}/bootstrap_${i}/logs/acceptance_matrix.out" /N/u/rdube/Quartz/work/moments_workflow/acceptance_matrix.slurm ${dataset} ${maxL} ${i})
    #submit unsampled acceptance correcting job
    jobs_to_wait_for_acceptance_correcting="${data_job_id}:${accMC_job_id}:${genMC_job_id}:${acceptance_matrix_job_id}"
